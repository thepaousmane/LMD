...
// echo $_SESSION['name_section'];
 ...
 $sql=$conn2->prepare("SELECT * from specialite_departement");
 $sql->execute();
 while($row = $sql->fetch(PDO::FETCH_ASSOC)) 
 { 
  $nom_departement=$row['nom_specialite'];
  $i=$row['id'];

    if(!empty($_POST['nom_specialite_D'.$i]))
    {
      $_SESSION['nom_specialite_D']= $_POST['nom_specialite_D'.$i];
      echo "Nom de la specialite : ".$_SESSION['nom_specialite_D'];
      echo"<br>";
      echo "Nom du département : ".$_SESSION['name_departement'];
      echo"<br>";
  

      $conn=new PDO("mysql:host=127.0.0.1;dbname=upa;charset=utf8","root");
      $sql=$conn->prepare("SELECT duree_formation_specialite_D from specialite_departement where id ='".$i."' ");
      $sql->execute();
      $data=$sql->fetch();

      // echo $data['nom_section'];
       $_SESSION['df_sp_D']=$data['duree_formation_specialite_D'];

          for($cpt=1;$cpt<=$_SESSION['df_sp_D'];$cpt++)
          {
            ?>
          <form action="afficher_classe_D.php" method="post">
            <input type="submit" name="annee<?php echo $cpt; ?>" value="annee_<?php echo $cpt; ?>">
          </form>
    <?php }
    }
}
...
// echo $_SESSION['name_section'];
 ...
 while($row = $sql->fetch(PDO::FETCH_ASSOC)) 
 { 
  $nom_section=$row['nom_specialite'];
  $i=$row['id'];

    if(!empty($_POST['nom_specialite'.$i]))
    {
      echo"toto";
      $_SESSION['nom_specialite']= $_POST['nom_specialite'.$i];
      ...
      $conn=new PDO("mysql:host=127.0.0.1;dbname=upa;charset=utf8","root");
      $sql=$conn->prepare("SELECT duree_formation_specialite from specialite_section where id ='".$i."' ");
      $sql->execute();
      $data=$sql->fetch();

      // echo $data['nom_section'];
       $_SESSION['df_sp']=$data['duree_formation_specialite'];
...
?>
   	...
   	<!-------------------------------------------------------PARTIE DES BOUTONS POUR SECTION------------------------------------------------------------------------------------------------->
     <?php 
if(empty($_POST['delete_specialite']))
    {  ?>
    <form action="directeur.php" method="post">
          <input type="submit" name="delete_specialite" value="Supprimer la specialité">
    </form>
<?php } ?>
<?php   if(!empty($_POST['nom_specialite'.$i]))
    {
      $_SESSION['nom_specialite']=$_POST['nom_specialite'.$i];
    $nom_specialite=$_SESSION['nom_specialite'];
    // $replaced1 = str_replace(' ', '_', $nom_specialite1);
    // $nom_specialite = str_replace("'", "_", $replaced1);

    $conn=new PDO("mysql:host=127.0.0.1;dbname=upa;charset=utf8","root");
    $sql=$conn->prepare("SELECT duree_formation_specialite from specialite_section where nom_specialite ='".$nom_specialite."' ");
    $sql->execute();
    $data=$sql->fetch();
    $_SESSION['DFS']=$data['duree_formation_specialite'];
    if($_SESSION['DFS']<3)
    {
    ?>
    <form action="niveau.php" method="post">
          <input type="submit" name="ajouter_annee" value="Ajouter une année supplémentaire">
    </form>
<?php }
  }
?>
<?php
if(!empty($_POST['ajouter_annee']))
{
  $nom_specialite=$_SESSION['nom_specialite'];
  $replaced1 = str_replace(' ', '_', $nom_specialite1);
  $nom_specialite = str_replace("'", "_", $replaced1);
  ...
  $sql4=$conn4->prepare("UPDATE specialite_section SET duree_formation_specialite ='".$DFS."' where nom_specialite='".$nom_specialite."'  ");    
  $sql4->execute();
}
?>

<?php 
if(empty($_POST['delete_last_year'])&&(!empty($_SESSION['DFS'])))
    { 
      if($_SESSION['DFS']>1){
      ?>
<form action="niveau.php" method="post">
      <input type="submit" name="delete_last_year" value="Supprimer derniere année de formation">
</form>
...
<?php 
if(!empty($_POST['delete_last_year']))
    {  
      $nom_specialite=$_SESSION['nom_specialite'];
      $replaced1 = str_replace(' ', '_', $nom_specialite1);
      $nom_specialite = str_replace("'", "_", $replaced1);
      ...
      $DFS=$_SESSION['DFS'];
        ...
        $sql4=$conn4->prepare("UPDATE specialite_section SET duree_formation_specialite ='".$DFS."' where nom_specialite='".$nom_specialite."'  ");    
        $sql4->execute();
        echo '<meta http-equiv="refresh" content="3;URL=\'http://localhost/UPA/directeur.php\'">';
    }
      ?>
...
//   // echo '<meta http-equiv="refresh" content="3;URL=\'http://localhost/UPA/indexFiliere.php\'">';
...
//         ?>
     <!-- <form action="niveau.php" method="post">
       ...
       <input type="submit" name="semestre<?php 
      //  echo $o; 
       ?>" value="semestre_<?php 
      //  echo $o; 
       ?>">
