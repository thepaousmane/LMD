...
<?php
    $i=$_SESSION['ii'];
    $name_departement_table=$_SESSION['name_departement_transf'];
    ...
    if( (!empty($_POST['modif_bdd_name'.$i]))&&(!empty($_POST['modif_bdd_prenom'.$i]))&&(!empty($_POST['modif_bdd_adresse'.$i]))
    &&(!empty($_POST['modif_bdd_date_naiss'.$i]))&&(!empty($_POST['modif_bdd_tel'.$i]))
    &&(!empty($_POST['modif_bdd_password'.$i])) )
{
    $_SESSION['md_bdd_tel'.$i]=$_POST['modif_bdd_tel'.$i];
    $_SESSION['md_bdd_date_naiss'.$i]=$_POST['modif_bdd_date_naiss'.$i];
    $_SESSION['md_bdd_adresse'.$i]=$_POST['modif_bdd_adresse'.$i];
    $_SESSION['md_bdd_nom'.$i]=$_POST['modif_bdd_name'.$i];
    $_SESSION['md_bdd_prenom'.$i]=$_POST['modif_bdd_prenom'.$i];
    $_SESSION['modif_bdd_password'.$i]=$_POST['modif_bdd_password'.$i];
    
    $format_nom=$_POST['modif_bdd_name'.$i];
    $format_prenom=$_POST['modif_bdd_prenom'.$i];
    $format_nom=mb_strtoupper($format_nom);
    // $format_prenom = ucwords(strtolower($format_prenom));
    $format_prenom = ucwords($format_prenom);
    // $format_prenom = strtolower($format_prenom);
    $_SESSION['format_md_bdd_nom'.$i]=$format_nom;
    $_SESSION['format_md_bdd_prenom'.$i]=$format_prenom;
    // $_SESSION['nom_domaine']='@esp.sn';//******************Revoir la gestion du nom de domaine c'est pas censé se passer comme ça ***************************************/
    $email=$_POST['modif_bdd_prenom'.$i].'.'.$_POST['modif_bdd_name'.$i].$i.$_SESSION['nom_domaine'];
...
<form action="modifier_gestion_professeur.php" method="post">
  ...
  <td><input disabled type="text" 
   value="<?php if(!empty($_POST['modif_bdd_name'.$i])){ echo $format_nom; } ?>"/></td>

  <td><input disabled type="text" 
   value="<?php if(!empty($_POST['modif_bdd_prenom'.$i])){ echo $format_prenom; } ?>"/></td>

   <td><input disabled type="text" value="<?php echo $_SESSION['email'.$i]; ?>"/></td>

   <td><input disabled type="text" value="<?php if(!empty($_POST['modif_bdd_password'.$i])){ echo $_POST['modif_bdd_password'.$i]; } ?>"/></td>

  <td><input disabled type="text" 
   value="<?php if(!empty($_POST['modif_bdd_tel'.$i])){ echo $_POST['modif_bdd_tel'.$i]; } ?>"/></td>

  <td><input disabled type="date" 
   value="<?php if(!empty($_POST['modif_bdd_date_naiss'.$i])){ echo $_POST['modif_bdd_date_naiss'.$i]; } ?>"/></td>

  <td><input disabled type="text" 
   value="<?php if(!empty($_POST['modif_bdd_adresse'.$i])){ echo $_POST['modif_bdd_adresse'.$i]; } ?>"/></td>
...
<form action="gestion_professeur.php" method="post">
    <input type="submit" name="modifier<?php echo $i; ?>" value="modifier les données"/>
</form>
...
{
    $nom=$_SESSION['format_md_bdd_nom'.$i];
    $prenom=$_SESSION['format_md_bdd_prenom'.$i];
    $adresse=$_SESSION['md_bdd_adresse'.$i];
    $date_naiss=$_SESSION['md_bdd_date_naiss'.$i];
    $tel=$_SESSION['md_bdd_tel'.$i];
    $email=$_SESSION['email'.$i];
    $password=$_SESSION['modif_bdd_password'.$i];
    ...
    try{
        ...
        // $connxx4=new PDO("mysql:host=127.0.0.1;dbname=upa;charset=utf8","root");
            $sqlx4=$connxx4->prepare("UPDATE compte_professeur_departement_".$name_departement_table."
             SET nom ='".$nom."', prenom ='".$prenom."', adresse ='".$adresse."', date_de_naissance ='".$date_naiss."',
    ...
    echo "les données ont bien été mis à jour dans la base de données";
?>
        <script type="text/javascript">
                window.setTimeout('window.location="http://localhost/UPA/gestion_professeur.php"; ',2000);
